...
```{r data format}
## try this out with Super Tuesday states
# first create ratio variable
q_id_ST <- polls_ST %>%
...
```{r ratio CI}
## is this right? just exponentiating bounds?
## maybe try doing bootstrap on ratio itself
ggplot(theta_ST_mid_df, aes(x=ST)) +
...
## ratio CIs
ratio_ST_0_mid <- exp(theta_ST_0_mid)
ratio_ST_0_mid_lb <- rep(0, length(ST))
ratio_ST_0_mid_ub <- rep(0, length(ST))
for (i in 1:length(ST)){
  ratio_ST_0_mid_lb[i] <- ratio_ST_0_mid[i] - 1.96*sqrt(boot_r_ST_var[i])
  ratio_ST_0_mid_ub[i] <- ratio_ST_0_mid[i] + 1.96*sqrt(boot_r_ST_var[i])
}
...
## Lil plot
ratio_ST_mid_df <- data.frame(est = ratio_ST_0_mid, res = primary$ratio_res, 
                              lb = ratio_ST_0_mid_lb, ub = ratio_ST_0_mid_ub,
                              state = ST)

ratio_coverage <- mean(ratio_ST_mid_df$res < ratio_ST_mid_df$ub & ratio_ST_mid_df$res > ratio_ST_mid_df$lb)

ggplot(ratio_ST_mid_df, aes(x=ST)) +
  geom_hline(yintercept = 1, color = "grey75") +
  geom_point(aes(y = est)) +
  geom_errorbar(aes(ymin = lb, ymax = ub), width = 0.2) +
  geom_point(aes(y = res), color = "blue", shape = 17, size = 2) +
...
# plot is different from lazy back-transformation
# is this right?
```
...
## figure out how to interpret beta hats
...
# SEs for spread --> not sure how to do for theta
pred538 %>%
